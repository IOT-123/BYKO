...
(function() {
    ...
    var GeoSim = function(params) {
        ...
        var _watchTimer = null,
            ...
            _speed = (params.speed || 18) / SECONDS_IN_HOUR, // km/second (defaults to 40kmh or 25mph)
            _current = { 
                coords: { 
                    latitude: 0,
                    longitude: 0,
        ...
        function nextCoord(callback) {
            ...
            if(_index < _coords.length - 1) {
                ...
                lastLat = _current.coords.latitude;
                lastLng = _current.coords.longitude;
                _current.coords.latitude = coord.latitude;
                _current.coords.longitude = coord.longitude;
                _current.coords.speed = _speed * 1000 + getRandomArbitrary(-5, 5);
                _current.coords.heading = Math.atan2(coord.latitude - lastLat, coord.longitude - lastLng) * 180 / Math.PI;
                //set rate of change
